...
void PadPlateau::genererTuile(int ligne, int colonne, bool type)
{
  Case portePos[4] = {Case(2), Case(4), Case(11), Case(13)};
  Couleur couleur[4] = {Couleur::JAUNE, Couleur::VERT, Couleur::ORANGE, Couleur::VIOLET};
  std::vector<Case> objectifs;
  std::string key = std::to_string(ligne) + "_" + std::to_string(colonne);
  for(int i = 0; i < 24; i++)
  {
  ...
  // depart == 0
  {
    ...
    std::shuffle(&pionPose[0], &pionPose[4], *rng);

    for(int i = 0; i < 4; i++)
    {
      ajouter_porte(ligne, colonne, portePos[i], couleur[i]);
      d_tuiles[key].portes[couleur[i]] = portePos[i].index();
  ...
  {
    bool sort = ((*rng)() % 2) == 1;
    ajouter_porte(ligne, colonne, portePos[3], Couleur::AUCUNE);

    d_tuiles[key].portes[Couleur::AUCUNE] = portePos[3].index();

    objectifs.push_back(portePos[3]);
    int nbPorte = ((*rng)()%2)+1;
    Couleur coul = couleur[(*rng)()%4];
    std::shuffle(&portePos[0], &portePos[3], *rng);
    ...
    if(sort) 
    {
      ...
      d_tuiles[key].sorties[coul] = c.index();
    }
    else 
    {
  ...
  for(int i = 0; i < 16; i++)
  {
    ...
    murUnion.tab.push_back(i);
    
  }

  shuffle(murTab.begin(), murTab.end(), *rng);

  for(int i = 0; i < 24; i++)
  {
    if(murUnion.trouver_racine(murTab[i][0].index()) != murUnion.trouver_racine(murTab[i][1].index()))
    {
      abattre_mur(ligne, colonne, murTab[i]);
      murUnion.union_ensembles(murTab[i][0].index(), murTab[i][1].index());
    }
    
    int equal = true;
    int val = murUnion.tab.at(objectifs[0].index());
    for(int j = 1; j < (int)objectifs.size(); j++)
    {
      if(murUnion.tab.at(objectifs[j].index()) != val)
      {
        equal = false;
        break;
      }
